...
public class ForeignCom extends AppCompatActivity implements Runnable ,AdapterView.OnItemClickListener, AdapterView.OnItemLongClickListener{
    ...
    private String logDateB = "";
    private final String  DATE_SP_KEY = "lastInfoStr";
    java.util.List<String> myInfo = new ArrayList<String>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_b_a_com);

        //获取时间
        SharedPreferences sp = getSharedPreferences("ba", Context.MODE_PRIVATE);
        logDateB = sp.getString(DATE_SP_KEY,"");
        Log.i(TAG, "lastInfoStr: "+logDateB);
        ...
        handler = new Handler(){
            ...
            public void handleMessage(@NonNull Message msg) {
                ...
                if (msg.what == 1){
                    ...
                    ListView listView = (ListView) findViewById(R.id.lv_comb);
                    ListAdapter adapter = new ArrayAdapter<String>(ForeignCom.this,android.R.layout.simple_list_item_1,liList);
    ...
    public void run() {
        ...
        Log.i(TAG, "curDateStr: "+curDateStrM+"，logDate"+logDateB);

        //判断是从哪里获取数据
        if (curDateStrM.equals(logDateB)){
            //从数据库中获得数据
            Log.i(TAG, "run: 日期相等，从数据库中获得数据");
            InfoManager infoManager = new InfoManager(ForeignCom.this);
            for (InfoItem item:infoManager.listAll(DBHelper.TB_NAME5)){
            ...
            int page = 0;
            try {
                URL url = new URL("https://jmwy.swufe.edu.cn/index/zytz.htm");
            ...
            try {
                //首页数据提取
                Log.i(TAG, "run: 首页数据");
                String urlFirst = "https://jmwy.swufe.edu.cn/index/zytz.htm";
                ...
                int size1,size2;
                int i = 1;
                String url = "https://jmwy.swufe.edu.cn/index/zytz.htm";
                for (;i<page+1;i++){
            ...
            //把数据写入数据库
            InfoManager infoManager = new InfoManager(ForeignCom.this);
            infoManager.deleteAll(DBHelper.TB_NAME5);
            infoManager.addAll(infoItemsList,DBHelper.TB_NAME5);
    ...
    public void showProgressDialog(String title, String message) {
        if (progressDialog == null) {
            progressDialog = ProgressDialog.show(ForeignCom.this, title, message, true, false);
        ...
        builder.setTitle("提示").setMessage("是否添加到我的消息中？").setPositiveButton("是", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (myInfo.indexOf(toMy) !=-1 ){
                    Toast.makeText(ForeignCom.this,"不可重复添加",Toast.LENGTH_SHORT).show();
                    Log.i(TAG, "onClick: 阻止重复添加");
                }else {
                    infoManager.add(new InfoItem(toMy,myURL),"tb_c"+tbNum);
                    Toast.makeText(ForeignCom.this,"已添加",Toast.LENGTH_SHORT).show();
    ...
    public void search(View btn){
        resultList.clear();
        //获取关键字
        EditText input = findViewById(R.id.key_words_b);
